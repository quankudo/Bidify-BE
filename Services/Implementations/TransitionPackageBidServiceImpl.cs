...
namespace bidify_be.Services.Implementations
{
    public class TransitionPackageBidServiceImpl : ITransitionPackageBidService
    {
        ...
        private readonly IValidator<TransitionPackageBidRequestValidator> _validator;
        public TransitionPackageBidServiceImpl(
            IUnitOfWork unitOfWork, 
            ICurrentUserService currentUserService, 
            ILogger<TransitionPackageBidServiceImpl> logger, 
            UserManager<ApplicationUser> userManager,
            IValidator<TransitionPackageBidRequestValidator> validator)
        {
        ...
        public async Task<TransitionPackageBid> CreateAsync(TransitionPackageBidRequest request)
        {
            ...
            // Begin transaction
            await using var transaction = await _unitOfWork.BeginTransactionAsync(); // giả sử unitOfWork có transaction support
            ...
            try
            {
                ...
                // Cập nhật user balance & bid count
                user.Balance -= packageBid.Price;
                ...
                _logger.LogInformation("TransitionPackageBid created successfully for UserId {UserId}, PackageBidId {PackageBidId}.", userId, request.PackageBidId);

                return transitionPackageBid;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error creating TransitionPackageBid for PackageBidId {PackageBidId}.", request.PackageBidId);
        ...
        public async Task<List<TransitionPackageBidResponse>> GetAllByUserIdAsync()
        {
            var userId = _currentUserService.GetUserId();
            _logger.LogInformation("Fetching all TransitionPackageBids for UserId {UserId}", userId);

            var bids = await _unitOfWork.TransitionPackageBidRepository.GetByUserIdAsync(userId);
